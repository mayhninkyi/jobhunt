    ...
    <div>
        ...
        <v-data-table class="elevation-4" :headers="headers" :items="jobList" :items-per-page="5">
            <template v-slot:[`item.actions`]="{ item }">
                <v-btn class="mr-2" color="primary" fab x-small elevation="2" @click="editDialog = true; toUpdateJob = item; category = toUpdateJob.category.id;
">
            ...
            <v-card>
                ...
                <v-card-text class="mt-8">
                    <v-form ref="updateJobForm" v-model="updateJobForm">
                        <!-- Title -->
                        <v-text-field v-model="toUpdateJob.title" :counter="50" :rules="[
    (v) => !!v || 'Required',
    (v) =>
        (v && v.length <= 50) || 'Title must be less than 50 characters',
                        ...
                        <!-- Description -->
                        <v-text-field v-model="toUpdateJob.description" :rules="[
    (v) => !!v || 'Required',
]" label="Description" required></v-text-field>

                        <!-- Requirement -->
                        <v-text-field v-model="toUpdateJob.requirement" :rules="[
    (v) => !!v || 'Required',
]" label="Requirement" required></v-text-field>

                        <!--No of Employee-->
                        <v-text-field v-model="toUpdateJob.noOfEmployee" type="number" :rules="[
    (v) => !!v || 'Required',
]" label="No: of Employee" required></v-text-field>

                        <!--Salary-->
                        <v-text-field v-model="toUpdateJob.salary" type="number" :rules="[
    (v) => !!v || 'Required',
